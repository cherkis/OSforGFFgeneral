...
**Reflection positivity inner product.** For the free covariance $C(x,y) = \frac{1}{2\pi} K_0(m\lVert x-y\rVert)$:

$$\langle \Theta f, f \rangle_C := \int_{\mathbb{R}^2} \int_{\mathbb{R}^2} \overline{f(\Theta x)}\  C(x,y)\  f(y)\  dx\  dy$$
...
$$H(s, \|z\|) = \frac{1}{(2\pi)^2} \int_{\mathbb{R}^2} e^{ik\cdot z}\  e^{-s\|k\|^2}\  dk$$
...
$$\langle\Theta f, f\rangle_C = \frac{1}{2(2\pi)} \int_{\mathbb{R}} \frac{1}{\omega}\  (\text{inner integral})\  d\vec{k}$$
...
$$F_\omega(\vec{k}) = \int_{\mathbb{R}^2} f(x)\  e^{-\omega x_0}\  e^{-i\vec{k}\cdot\vec{x}}\  dx$$
...
$$\langle\Theta f, f\rangle_C = \frac{1}{2(2\pi)} \int_{\mathbb{R}} \frac{1}{\omega(\vec{k})}\  |F_\omega(-\vec{k})|^2\  d\vec{k}$$
