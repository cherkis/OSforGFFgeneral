...
**Clustering.** For any real test functions $f, g$ and $\varepsilon > 0$, there exists $R > 0$ such that for all translations $a \in \mathbb{R}^2$ with $\|a\| > R$:
...
3. The **cross term decays** as $\|a\| \to \infty$ because the propagator $C(x-y) \sim K_0(m\|x-y\|)$ decays exponentially at large distances, and the Schwartz test functions provide rapid localization.
...
|-------------|-------------|
| [`timeTranslationE`](../OSforGFF/OS4_MGF.lean#L146) | $T_s \in E(2)$: $T_s(x) = x + se_0$ |
