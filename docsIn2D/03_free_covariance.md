...
The free Euclidean propagator for a massive scalar field with mass $m > 0$ in $d = 2$ dimensions is defined in momentum space as:

$$\hat{C}(k) = \frac{1}{\|k\|^2 + m^2}$$

The position-space kernel is obtained by inverse Fourier transform:

$$C(x - y) = \frac{1}{(2\pi)^2} \int_{\mathbb{R}^2} \frac{e^{ik\cdot(x-y)}}{\|k\|^2 + m^2}\  d^2k$$

In 2D, this evaluates to the Bessel function representation:

$$C(x, y) = \frac{1}{2\pi} K_0(m\|x - y\|)$$

where $K_0$ is the modified Bessel function of the second kind. Key properties:
- **Positivity:** $C(x, y) > 0$ for $x \ne y$
- **Symmetry:** $C(x, y) = C(y, x)$
- **Euclidean invariance:** $C(gx, gy) = C(x, y)$ for all $g \in E(2)$
- **Singularity:** $C(x, y) \sim -\text{const} \cdot \log\|x-y\|$ as $x \to y$
- **Exponential decay:** $C(x, y) \sim \text{const} \cdot e^{-m\|x-y\|}/\|x-y\|^{1/2}$ as $\|x-y\| \to \infty$
...
where $H_t(r) = \frac{1}{4\pi t} e^{-r^2/(4t)}$ is the 2D heat kernel.

### 2. Regularization Strategy

To rigorously justify Fubini-type exchanges of integration, a Gaussian regulator $e^{-\alpha\|k\|^2}$ is introduced:

$$C_\alpha(x, y) = \int \frac{e^{-\alpha\|k\|^2} e^{ik\cdot(x-y)}}{\|k\|^2 + m^2} \frac{dk}{(2\pi)^2}$$
...
For test functions $f, g \in \mathcal{S}(\mathbb{R}^2, \mathbb{C})$, the bilinear covariance form is:
...
| [`freeCovarianceBessel`](../OSforGFF/CovarianceMomentum.lean#L462) | Bessel representation: $C(x,y) = \frac{1}{2\pi} K_0(mr)$ |
...
| [`heatKernelPositionSpace`](../OSforGFF/CovarianceMomentum.lean#L236) | 2D heat kernel $H_t(r)$ |
...
| [`covarianceSchwingerRep_eq_besselFormula`](../OSforGFF/CovarianceMomentum.lean#L437) | $C^{\text{Schwinger}} = \frac{1}{2\pi} K_0(mr)$ |
...
| [`freeCovarianceKernel_decay_bound`](../OSforGFF/CovarianceMomentum.lean#L1731) | $\lvert C(z)\rvert \le \text{const} \cdot (1 + \lvert\log\lVert z\rVert\rvert)$ |
...
| [`freeCovariance_euclidean_invariant`](../OSforGFF/Covariance.lean#L403) | $C(gx, gy) = C(x, y)$ for $g \in E(2)$ |
