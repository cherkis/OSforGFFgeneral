...
**Theorem (OS1).** There exist $p \in [1,2]$ and $c > 0$ such that for all $f \in \mathcal{S}(\mathbb{R}^2, \mathbb{C})$:
...
**Part 2 (Local integrability).** The two-point function is $S_2(x) = (1/(2\pi))K_0(m\|x\|)$, which has a logarithmic singularity $\sim -\log\|x\|$ near the origin. Since $\log\|x\|$ is locally integrable in 2 dimensions, local integrability follows.
...
| [`schwinger_two_point_decay_bound_GFF`](../OSforGFF/OS1_GFF.lean#L145) | $\lvert S_2^{\mathrm{GFF}}(x-y)\rvert \le C(1 + \lvert\log\lVert x-y\rVert\rvert)$ |
| [`schwinger_two_point_decay_bound`](../OSforGFF/OS1_GFF.lean#L162) | $\lvert S_2(x-y)\rvert \le C(1 + \lvert\log\lVert x-y\rVert\rvert)$ |
...
**Step 5: Decay bound.**
The GFF two-point function is $S_2^{\mathrm{GFF}}(x) = C_{\mathrm{free}}(x) = (1/(2\pi))K_0(m\|x\|)$, where $K_0$ is the modified Bessel function of the second kind.

From Bessel function asymptotics ($K_0(z) \sim -\log(z/2)$ near the origin, exponential decay at infinity), the two-point function has a logarithmic singularity. This is proved in `schwinger_two_point_decay_bound_GFF` using `freeCovarianceKernel_decay_bound` from `CovarianceMomentum.lean`, which relies on `besselK0_near_origin_bound` and `besselK0_asymptotic`.

**Lean theorem**: `schwinger_two_point_decay_bound`

**Step 6: Local integrability in 2D.**
In $d = 2$ dimensions, the logarithmic singularity $\log\|x\|$ is locally integrable:

$$\int_{\|x\| < R} |\log\|x\||\  dx = 2\pi \cdot \int_0^R |\log r| \cdot r\  dr < \infty$$
