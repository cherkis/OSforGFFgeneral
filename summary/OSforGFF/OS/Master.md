...
> **⚠ Stale (pre–Stage G)**: predates the uniform-`d` refactor — still lists the retired `μ_GFF`/`μ_GFF2`/`μ_GFF3` shapes and "recovered verbatim" framing, and omits the `_dim5`/`_of_dim` headlines. The Lean source is authoritative; regenerate with `/lean-summarize`.
