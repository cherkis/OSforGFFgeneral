...
open Set Metric Real in
/-- ‖x‖^{-(1:ℝ)} is integrable on ball(0,R) in ℝ² for any R > 0 (since 1 < 2 = dim). -/
private lemma rpow_neg_one_integrableOn_ball (R : ℝ) (hR : 0 < R) :
    IntegrableOn (fun (x : EuclideanSpace ℝ (Fin 2)) => ‖x‖ ^ (-(1:ℝ)))
      (ball (0 : EuclideanSpace ℝ (Fin 2)) R) volume := by
  apply integrableOn_ball_of_rpow_decay (d := 2) (by omega) (by norm_num : (0:ℝ) < 1)
    (by norm_num : (1:ℝ) < 2) hR
  · intro x; rw [abs_of_nonneg (rpow_nonneg (norm_nonneg x) _), one_mul]
  · exact Measurable.aestronglyMeasurable (by fun_prop)

/-- |log r| + 1 ≤ r⁻¹ + r + 1 for r ≥ 0 (uses log x ≤ x - 1 and log(1/r) ≤ 1/r - 1). -/
private lemma abs_log_add_one_le (r : ℝ) (hr : 0 ≤ r) : |Real.log r| + 1 ≤ r⁻¹ + r + 1 := by
  rcases eq_or_lt_of_le hr with rfl | hr
  · simp [Real.log_zero]
  · rcases le_or_gt r 1 with hr1 | hr1
    · have h1 := Real.log_le_sub_one_of_pos (inv_pos.mpr hr)
      rw [Real.log_inv] at h1
      rw [abs_of_nonpos (Real.log_nonpos hr.le hr1)]; linarith
    · rw [abs_of_pos (Real.log_pos hr1)]
      linarith [Real.log_le_sub_one_of_pos hr, inv_pos.mpr hr]

/-- r^{-(1:ℝ)} = r⁻¹ for r ≥ 0 (rpow agrees with field inverse). -/
private lemma rpow_neg_one_eq_inv (r : ℝ) (hr : 0 ≤ r) : r ^ (-(1:ℝ)) = r⁻¹ := by
  rcases eq_or_lt_of_le hr with rfl | hr
  · simp [Real.zero_rpow (by norm_num : (-(1:ℝ)) ≠ 0)]
  · rw [Real.rpow_neg hr.le, Real.rpow_one]

open Set Metric Real in
/-- Functions with logarithmic singularity are locally integrable in 2D.
    For f : ℝ² → ℝ with |f(x)| ≤ C * (|log ‖x‖| + 1), f is locally integrable.
    This is the d=2 analogue of `locallyIntegrable_of_rpow_decay_real` (which requires d ≥ 3).

    The proof dominates |log ‖x‖| by ‖x‖⁻¹ (which is integrable on balls in 2D since 1 < 2)
    using the inequality |log r| ≤ 1/r + r for r ≥ 0. -/
theorem locallyIntegrable_of_log_singularity
    {f : EuclideanSpace ℝ (Fin 2) → ℝ} {C : ℝ}
    (hC : C > 0)
    (h_decay : ∀ x, |f x| ≤ C * (|Real.log ‖x‖| + 1))
    (h_meas : AEStronglyMeasurable f volume) :
    LocallyIntegrable f volume := by
  intro x₀
  -- For any x₀, dominate f on a ball containing x₀ by C * (‖x‖⁻¹ + R + 1)
  let R := ‖x₀‖ + 2
  have hR : 0 < R := by positivity
  have hx₀_in : x₀ ∈ ball (0 : EuclideanSpace ℝ (Fin 2)) R := by
    simp only [mem_ball, dist_zero_right]; linarith
  refine ⟨ball (0 : EuclideanSpace ℝ (Fin 2)) R, isOpen_ball.mem_nhds hx₀_in, ?_⟩
  -- Build the integrable dominating function: C * ‖x‖^{-1} + C*(R+1)
  have h_rpow : IntegrableOn (fun x : EuclideanSpace ℝ (Fin 2) => C * ‖x‖ ^ (-(1:ℝ)))
      (ball (0 : EuclideanSpace ℝ (Fin 2)) R) volume :=
    (rpow_neg_one_integrableOn_ball R hR).const_mul C
  have h_const : IntegrableOn (fun _ : EuclideanSpace ℝ (Fin 2) => C * (R + 1))
      (ball (0 : EuclideanSpace ℝ (Fin 2)) R) volume :=
    integrableOn_const (measure_ball_lt_top.ne)
  have h_dom : IntegrableOn (fun x : EuclideanSpace ℝ (Fin 2) =>
      C * ‖x‖ ^ (-(1:ℝ)) + C * (R + 1))
      (ball (0 : EuclideanSpace ℝ (Fin 2)) R) volume :=
    h_rpow.add h_const
  -- f is dominated a.e. on the ball
  apply h_dom.mono' h_meas.restrict
  rw [ae_restrict_iff' measurableSet_ball]
  apply ae_of_all
  intro x hx
  rw [Real.norm_eq_abs]
  have hxR : ‖x‖ < R := by simp [mem_ball, dist_zero_right] at hx; exact hx
  have h_eq : ‖x‖ ^ (-(1:ℝ)) = ‖x‖⁻¹ := rpow_neg_one_eq_inv ‖x‖ (norm_nonneg x)
  calc |f x| ≤ C * (|Real.log ‖x‖| + 1) := h_decay x
    _ ≤ C * (‖x‖⁻¹ + R + 1) := by
        apply mul_le_mul_of_nonneg_left _ hC.le
        linarith [abs_log_add_one_le ‖x‖ (norm_nonneg x)]
    _ = C * ‖x‖ ^ (-(1:ℝ)) + C * (R + 1) := by rw [h_eq]; ring

/-- **Polynomial decay is integrable in d-1 dimensions**: The function 1/(1+‖x‖)^4 is integrable
    over SpatialCoords = EuclideanSpace ℝ (Fin (d-1)).

    For d=2, the spatial dimension is 1, and decay rate 4 > 1 ensures integrability.

    **Mathematical content**: In ℝ^{d-1} with spherical coordinates,
    ∫ 1/(1+r)^4 · r^{d-2} dr dΩ < ∞
    since the integrand decays faster than r^{-(d-1+1)} for large r.
