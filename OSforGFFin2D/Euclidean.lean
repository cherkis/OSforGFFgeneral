...
This file implements the Euclidean group E(d) = ℝᵈ ⋊ O(d) and its actions on function spaces,
...
- `O4`: The orthogonal group O(d) as linear isometries on spacetime
- `E`: Euclidean group structure with rotation `R : O4` and translation `t : SpaceTime d`
...
/-OS2 R^d with d=2, where mu is the Lebesgue measure.
...
/-theorem ---------------------------------------------
     ...
     For all Euclidean motions g,h and every point x ∈ ℝᵈ we have
...
/-for all Euclidean motions g and h and any point x ∈ ℝᵈ, pulling x forward by the product g*h equals pulling by h first and then by g.
...
gR : O4 the rotation, gt : ℝᵈ the translation.
...
/-Result: we've established that act : E → (ℝᵈ → ℝᵈ) is a homomorphism into the function-composition monoid—exactly what OS-2 needs for its pull-back action on fields.-/
...
lemma measurePreserving_act (g : @E d) :
    MeasurePreserving (fun x : SpaceTime d => act g x) μ μ := by
  have rot : MeasurePreserving (fun x : SpaceTime d => g.R x) μ μ := by
    simpa using (g.R.toLinearIsometryEquiv rfl).measurePreserving
  have trans : MeasurePreserving (fun x : SpaceTime d => x + g.t) μ μ := by
...
/-- Action of Euclidean group on test functions via pullback.
    For g ∈ E and f ∈ TestFunctionℂ d, define (g • f)(x) = f(g⁻¹ • x).
...
/-- Action of Euclidean group on real test functions via pullback.
    For g ∈ E and f ∈ TestFunction d, define (g • f)(x) = f(g⁻¹ • x).
...
lemma euclidean_action_unified_basis (g : @E d) :
    MeasurePreserving (euclidean_pullback g) (μ : Measure (SpaceTime d)) μ := by
...
noncomputable def euclidean_action_L2 (g : @E d)
  ...
  have h_meas_pres : MeasurePreserving (euclidean_pullback g) (volume : Measure (SpaceTime d)) (volume : Measure (SpaceTime d)) :=
