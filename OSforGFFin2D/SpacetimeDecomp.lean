...
def spacetimeDecomp : SpaceTime STDimension ≃ᵐ ℝ × SpatialCoords STDimension :=
...
theorem spacetimeDecomp_measurePreserving :
    MeasurePreserving spacetimeDecomp (volume : Measure (SpaceTime STDimension)) volume := by
  unfold spacetimeDecomp
  -- Step 1: PiLp → (Fin d → ℝ) is measure-preserving
  have h1 : MeasurePreserving (piLpMeasurableEquiv STDimension)
      (volume : Measure (SpaceTime STDimension)) volume :=
...
theorem spacetimeDecomp_apply (k : SpaceTime STDimension) :
...
lemma spacetimeDecomp_symm_eq_spacetimeOfTimeSpace (t : ℝ) (v : SpatialCoords STDimension) :
  ...
  · -- i = j + 1: spatial components
    ...
    have h_spatialPart_def : ∀ (k : SpaceTime STDimension), spatialPart k j = k j.succ := fun _ => rfl
...
lemma spacetime_norm_sq_decompose (k : SpaceTime STDimension) :
    ‖k‖^2 = (k 0)^2 + ‖spatialPart k‖^2 := by
  -- Expand SpaceTime norm as sum over 2 components
  have hST : ‖k‖^2 = (k 0)^2 + (k 1)^2 := by
    rw [EuclideanSpace.norm_sq_eq, Fin.sum_univ_two]
    simp only [Real.norm_eq_abs, sq_abs]
  -- Expand SpatialCoords norm as sum over 1 component
...
lemma integral_spacetime_prod_split {f : ℝ → ℂ} {g : SpatialCoords STDimension → ℂ}
    (_hf : Integrable f) (_hg : Integrable g) :
    ∫ k : SpaceTime STDimension, f (k 0) * g (spatialPart k) =
    (∫ k₀ : ℝ, f k₀) * (∫ k_sp : SpatialCoords STDimension, g k_sp) := by
