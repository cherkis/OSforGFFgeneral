...
theorem fourier_plancherel_schwartz (g : TestFunctionℂ STDimension) :
...
noncomputable def SchwingerTwoPointFunction_GFF (m : ℝ) [Fact (0 < m)] (x : SpaceTime STDimension) : ℝ :=
  freeCovarianceKernel m x

/-- The GFF two-point function equals the free covariance kernel by definition. -/
theorem schwingerTwoPoint_eq_freeCovarianceKernel (m : ℝ) [Fact (0 < m)] (x : SpaceTime STDimension) :
...
theorem schwingerTwoPointFunction_eq_GFF (m : ℝ) [Fact (0 < m)] (x : SpaceTime STDimension) (hx : x ≠ 0) :
  ...
  have h_cont : ContinuousOn (freeCovarianceKernel m) {y : SpaceTime STDimension | y ≠ 0} :=
    freeCovarianceKernel_continuousOn m (Fact.elim ‹Fact (0 < m)›)
  have h_S₂ : ∀ (f g : TestFunction STDimension),
...
theorem schwingerTwoPointFunction_eq_freeCovarianceKernel (m : ℝ) [Fact (0 < m)] (x : SpaceTime STDimension)
    (hx : x ≠ 0) :
...
/-- The GFF two-point Schwinger function satisfies a logarithmic decay bound.
    For the free field in d=2, this follows from the K₀ Bessel function asymptotics:
    - Near origin: K₀(mr) ~ -log(mr), giving logarithmic divergence
    - Far from origin: K₀(mr) ~ exp(-mr), which is even faster decay -/
theorem schwinger_two_point_decay_bound_GFF (m : ℝ) [Fact (0 < m)] :
  ∃ C : ℝ, C > 0 ∧
    ∀ x y : SpaceTime STDimension,
...
/-- The abstract two-point Schwinger function satisfies a logarithmic bound.
    ...
    definition regularizes S(0) = 0 and |log(m·0)| + 1 = 1. -/
theorem schwinger_two_point_decay_bound (m : ℝ) [Fact (0 < m)] :
  ...
  refine ⟨C, hC_pos, fun x y => ?_⟩
  -- Split on coincident vs non-coincident points
  by_cases hxy : x - y = 0
  · -- Coincident points: S(0) = 0 by regularization
    rw [hxy, schwingerTwoPointFunction_zero, norm_zero]
    apply mul_nonneg hC_pos.le
    linarith [abs_nonneg (Real.log (m * ‖(0 : SpaceTime STDimension)‖))]
  · -- Non-coincident: abstract = concrete by bridge lemma
    rw [schwingerTwoPointFunction_eq_GFF m (x - y) hxy]
    exact hC_bound x y
...
theorem schwingerTwoPoint_measurable (m : ℝ) [Fact (0 < m)] :
  ...
  have h_ae_eq : (fun x => SchwingerTwoPointFunction (gaussianFreeField_free m) x) =ᶠ[ae volume]
    ...
    have h_singleton_null : (volume : Measure (SpaceTime STDimension)) {(0 : SpaceTime STDimension)} = 0 :=
      MeasureTheory.measure_singleton (0 : SpaceTime STDimension)
    -- The complement of {0} has full measure, so {x ≠ 0} ∈ ae volume
    have h_mem : {x : SpaceTime STDimension | x ≠ 0} ∈ ae volume := by
...
/-- The norm of the GFF generating function equals the exponential of minus one-half
    ...
    |exp(z)| = exp(Re z). -/
lemma gff_generating_norm_eq (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
...
lemma gff_generating_bound_by_imaginary (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
...
-/
lemma covariance_imaginary_L2_bound (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
  ...
  have h_prop_cont : Continuous fun k => freePropagatorMomentum_mathlib m k := by
    ...
    have hdenom_cont : Continuous fun k : SpaceTime STDimension => (2 * Real.pi)^2 * ‖k‖^2 + m^2 := by
...
lemma gff_generating_L2_bound (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
  ‖GJGeneratingFunctionalℂ (gaussianFreeField_free m) f‖ ≤
...
/-- The two-point Schwinger function is locally integrable.
    In d=2, the GFF covariance C(x) = (1/(2π))K₀(m‖x‖) has a logarithmic singularity
    K₀(z) ~ -log(z/2) - γ near z=0, which is locally integrable in 2D
    (∫₀^R |log r| r dr < ∞ in polar coordinates). -/
lemma gff_two_point_locally_integrable (m : ℝ) [Fact (0 < m)] :
  ...
  -- Get decay bound: |S(x-y)| ≤ C * (|log(m·‖x-y‖)| + 1)
  obtain ⟨C, hC_pos, hC_bound⟩ := schwinger_two_point_decay_bound m
  -- Convert from |log(m·‖x‖)| + 1 to |log ‖x‖| + 1 for locallyIntegrable_of_log_singularity
  -- Using: |log(mr)| ≤ |log m| + |log r|, so (|log m| + 1)(|log r| + 1) dominates
  set C' := C * (|Real.log m| + 1) with hC'_def
  have hC'_pos : C' > 0 := by positivity
  apply locallyIntegrable_of_log_singularity hC'_pos
  · intro x
    have h1 := hC_bound x 0
    simp only [sub_zero] at h1
    rw [Real.norm_eq_abs] at h1
    calc |SchwingerTwoPointFunction (gaussianFreeField_free m) x|
        ≤ C * (|Real.log (m * ‖x‖)| + 1) := h1
      _ ≤ C * ((|Real.log m| + |Real.log ‖x‖|) + 1) := by
          apply mul_le_mul_of_nonneg_left _ hC_pos.le
          rcases eq_or_lt_of_le (norm_nonneg x) with hx | hx
          · simp [← hx, Real.log_zero, mul_zero]
          · rw [Real.log_mul (ne_of_gt (Fact.out : 0 < m)) (ne_of_gt hx)]
            linarith [abs_add_le (Real.log m) (Real.log ‖x‖)]
      _ ≤ C' * (|Real.log ‖x‖| + 1) := by
          rw [hC'_def]
          have hab := mul_nonneg (abs_nonneg (Real.log m)) (abs_nonneg (Real.log ‖x‖))
          nlinarith [abs_nonneg (Real.log m), abs_nonneg (Real.log ‖x‖)]
