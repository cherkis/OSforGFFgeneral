...
private lemma freeCovarianceFormR_reflection_nonneg
  ...
  classical
  have hf_supp : ∀ x : SpaceTime STDimension, x 0 ≤ 0 → f.val x = 0 := fun x hx => by
...
lemma gaussianFreeField_real_generating_re
    (m : ℝ) [Fact (0 < m)] (h : TestFunction STDimension) :
