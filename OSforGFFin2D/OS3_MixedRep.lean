...
lemma gaussian_exp_factorize (s : ℂ) (k : SpaceTime STDimension) :
    Complex.exp (-s * ‖k‖^2) =
...
lemma k_integral_after_k0_eval (s : ℝ) (hs : 0 < s) (z : SpaceTime STDimension) :
    ∫ k : SpaceTime STDimension, Complex.exp (-Complex.I * ⟪k, z⟫_ℝ) * Complex.exp (-(s : ℂ) * ‖k‖^2) =
    (Real.sqrt (π / s) : ℂ) * Complex.exp (-(((z 0)^2 / (4 * s)) : ℝ)) *
      ∫ k_sp : SpatialCoords STDimension, Complex.exp (-Complex.I * spatialDot k_sp (spatialPart z)) *
                               Complex.exp (-(s : ℂ) * ‖k_sp‖^2) := by
  -- Step 1: Factor the integrand into k₀-part × k_sp-part using existing lemmas
  have h_factor : ∀ k : SpaceTime STDimension,
  ...
  have h_int_ksp : Integrable (fun k_sp : SpatialCoords STDimension =>
...
lemma timeReflection_sub_zero (x y : SpaceTime STDimension) :
    (timeReflection x - y) 0 = -(x 0) - y 0 := rfl

/-- The spatial part of (timeReflection x - y) equals spatialPart x - spatialPart y. -/
lemma spatialPart_timeReflection_sub (x y : SpaceTime STDimension) :
...
/-- **THEOREM**: Heat kernel bilinear form after k₀ integration.
    ...
    (2π)^{-2} ∫₀^∞ ∫_p̄ ∫∫ f̄(x)f(y) √(π/s) exp(-t²/(4s)) exp(-s(|p̄|² + m²)) exp(-ip̄·r̄) dx dy dp̄ ds
...
theorem heatKernel_bilinear_fourier_form (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
  ...
  have h_hk : ∀ s : ℝ, 0 < s → ∀ z : SpaceTime STDimension,
  ...
  have h_step1 : ∫ s in Set.Ioi 0, (Real.exp (-s * m^2) : ℂ) *
      ...
      ∫ s in Set.Ioi 0, (Real.exp (-s * m^2) : ℂ) *
        ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
          (starRingEnd ℂ (f x)) * f y *
          ((1 / (2 * π) ^ STDimension : ℝ) *
           ∫ k : SpaceTime STDimension, Complex.exp (-Complex.I * ⟪k, timeReflection x - y⟫_ℝ) *
  ...
  have h_step3 : ∫ s in Set.Ioi 0, (Real.exp (-s * m^2) : ℂ) *
    ...
    have h_ksp_reorder : ∀ x y : SpaceTime STDimension,
        (∫ k_sp : SpatialCoords STDimension,
          Complex.exp (-Complex.I * spatialDot k_sp (spatialPart x - spatialPart y)) *
          Complex.exp (-(s : ℂ) * ‖k_sp‖^2)) =
        (∫ k_sp : SpatialCoords STDimension,
  ...
  -- Step 6: Factor out (1/(2π)^2) from the s-integral
...
lemma omega_pos (k_sp : SpatialCoords STDimension) (m : ℝ) (hm : 0 < m) :
    0 < Real.sqrt (‖k_sp‖^2 + m^2) := by positivity

/-- The normalization constant relation for d=2:
    (1/(2π)²) × π = 1/(2(2π))

    Proof: (2π)² = 4π², so π/(4π²) = 1/(4π) = 1/(2·2π) -/
lemma normalization_constant_laplace :
    (1 / (2 * π) ^ 2 : ℝ) * π = 1 / (2 * (2 * π)) := by field_simp
...
lemma s_integral_complex_eval (k_sp : SpatialCoords STDimension) (x y : SpaceTime STDimension) (m : ℝ) (hm : 0 < m)
    (f : TestFunctionℂ STDimension) :
...
/-- **Laplace transform evaluation for the s-integral with normalization.**

    Combines two established facts:
    1. **s_integral_complex_eval** (proved): evaluates the inner s-integral pointwise
       √π · ∫₀^∞ s^{-1/2} exp(-t²/(4s) - sω²) ds = (π/ω) · exp(-ω|t|)
    2. **Normalization**: (1/(2π)^d) × π = 1/(2 × (2π)^{d-1}) for d = STDimension = 2 -/
theorem laplace_s_integral_with_norm (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
    (1 / (2 * π) ^ STDimension : ℝ) *
    ...
    (1 / (2 * (2 * π) ^ (STDimension - 1)) : ℝ) *
      ∫ k_spatial : SpatialCoords STDimension, ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
  ...
  -- Step 1: Swap ∫_s ↔ ∫_x ∫_y (Fubini) and evaluate the s-integral pointwise
  have h_s_eval : ∀ k_sp : SpatialCoords STDimension,
      ∫ s in Set.Ioi 0, ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
        (starRingEnd ℂ (f x)) * f y *
          ...
          Complex.exp (-Complex.I * spatialDot k_sp (spatialPart x - spatialPart y)) =
      ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
        (starRingEnd ℂ (f x)) * f y * (↑π / ↑(Real.sqrt (‖k_sp‖^2 + m^2))) *
          Complex.exp (-(|-(x 0) - y 0| : ℝ) * Real.sqrt (‖k_sp‖^2 + m^2)) *
          Complex.exp (-Complex.I * spatialDot k_sp (spatialPart x - spatialPart y)) := by
    intro k_sp
    -- Apply Fubini to swap ∫_s with ∫_x ∫_y
    rw [fubini_s_xy_swap m f k_sp]
    -- Apply s_integral_complex_eval pointwise under ∫_x ∫_y
    congr 1; ext x; congr 1; ext y
    exact s_integral_complex_eval k_sp x y m hm f
  simp_rw [h_s_eval]
  -- Step 2: Rewrite ↑π/↑ω as ↑π * ↑(1/ω) and factor ↑π out of the integrand
  have h_fac : ∀ (k : SpatialCoords STDimension) (x y : SpaceTime STDimension),
      (starRingEnd ℂ (f x)) * f y * (↑π / ↑(Real.sqrt (‖k‖^2 + m^2))) *
        Complex.exp (-(|-(x 0) - y 0| : ℝ) * Real.sqrt (‖k‖^2 + m^2)) *
        Complex.exp (-Complex.I * spatialDot k (spatialPart x - spatialPart y)) =
      ↑π * ((starRingEnd ℂ (f x)) * f y * ↑(1 / Real.sqrt (‖k‖^2 + m^2)) *
        Complex.exp (-(|-(x 0) - y 0| : ℝ) * Real.sqrt (‖k‖^2 + m^2)) *
        Complex.exp (-Complex.I * spatialDot k (spatialPart x - spatialPart y))) := by
    intro k x y
    have : (↑π / ↑(Real.sqrt (‖k‖^2 + m^2)) : ℂ) =
        ↑π * (↑(1 / Real.sqrt (‖k‖^2 + m^2)) : ℂ) := by
      rw [← Complex.ofReal_div]
      push_cast
      ring
    rw [this]; ring
  simp_rw [h_fac]
  -- Step 3: Factor ↑π out of the three nested integrals
  simp_rw [MeasureTheory.integral_const_mul]
  -- Now: ↑(1/(2π)^d) * (↑π * ∫...) = ↑(1/(2·(2π)^{d-1})) * ∫...
  -- Step 4: Combine ↑(1/(2π)^d) * ↑π into ↑((1/(2π)^d) * π) and prove normalization
  rw [← mul_assoc, ← Complex.ofReal_mul]
  -- Prove the normalization constant equality
  have h_norm : (1 / (2 * π) ^ STDimension : ℝ) * π = 1 / (2 * (2 * π) ^ (STDimension - 1)) := by
    simp only [show STDimension = 2 from rfl, show (2 : ℕ) - 1 = 1 from rfl, pow_one]
    exact normalization_constant_laplace
  rw [h_norm]
...
theorem schwinger_bilinear_integrable (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
    Integrable (fun (p : ℝ × SpaceTime STDimension × SpaceTime STDimension) =>
  ...
  have hf_int : Integrable (fun x => ‖f x‖) (volume : Measure (SpaceTime STDimension)) := f.integrable.norm
  have hf_L1 : Integrable f (volume : Measure (SpaceTime STDimension)) := f.integrable
  ...
  have h_heat_L1 : ∀ s > 0, ∫ z : SpaceTime STDimension, heatKernelPositionSpace s ‖z‖ = 1 :=
  ...
  let F : ℝ × SpaceTime STDimension × SpaceTime STDimension → ℂ := fun p =>
    (starRingEnd ℂ (f p.2.1)) * f p.2.2 *
    Real.exp (-p.1 * m^2) * heatKernelPositionSpace p.1 ‖timeReflection p.2.1 - p.2.2‖

  -- Define the real-valued dominating function
  let bound : ℝ × SpaceTime STDimension × SpaceTime STDimension → ℝ := fun p =>
    ‖f p.2.1‖ * Cf * Real.exp (-p.1 * m^2) * heatKernelPositionSpace p.1 ‖timeReflection p.2.1 - p.2.2‖

  -- The measure
  let μ : Measure (ℝ × SpaceTime STDimension × SpaceTime STDimension) :=
    (volume.restrict (Set.Ioi 0)).prod ((volume : Measure (SpaceTime STDimension)).prod volume)

  -- Pointwise bound: ‖F p‖ ≤ bound p for s > 0
  have h_bound : ∀ p : ℝ × SpaceTime STDimension × SpaceTime STDimension, p.1 ∈ Set.Ioi 0 →
      ‖F p‖ ≤ bound p := by
    intro p hp
  ...
  have h_bound_nonneg : ∀ p : ℝ × SpaceTime STDimension × SpaceTime STDimension, p.1 ∈ Set.Ioi 0 → 0 ≤ bound p := by
    intro p hp
  ...
  have h_bound_integrable : Integrable bound μ := by
    ...
    -- (b) For a.e. s: (x, y) ↦ bound(s, x, y) is integrable on SpaceTime STDimension × SpaceTime STDimension
  ...
  have h_null : μ {p : ℝ × SpaceTime STDimension × SpaceTime STDimension | p.1 ≤ 0} = 0 := by
    have h_preimage : {p : ℝ × SpaceTime STDimension × SpaceTime STDimension | p.1 ≤ 0} = Set.Iic 0 ×ˢ Set.univ := by
...
private def schwinger_tripleReorder :
    SpaceTime STDimension × (SpaceTime STDimension × ℝ) ≃ᵐ ℝ × (SpaceTime STDimension × SpaceTime STDimension) :=
...
private lemma measurePreserving_schwinger_tripleReorder :
    MeasurePreserving schwinger_tripleReorder
      ((volume : Measure (SpaceTime STDimension)).prod (volume.prod (volume.restrict (Set.Ioi 0))))
      ((volume.restrict (Set.Ioi 0)).prod (volume.prod volume)) := by
  ...
  have h1 : MeasurePreserving
      (MeasurableEquiv.prodAssoc (α := SpaceTime STDimension) (β := SpaceTime STDimension) (γ := ℝ)).symm
      ((volume : Measure (SpaceTime STDimension)).prod (volume.prod (volume.restrict (Set.Ioi 0))))
  ...
  have h2 : MeasurePreserving
      (MeasurableEquiv.prodComm (α := SpaceTime STDimension × SpaceTime STDimension) (β := ℝ))
      (((volume : Measure (SpaceTime STDimension)).prod volume).prod (volume.restrict (Set.Ioi 0)))
...
/-- **THEOREM** (converted from axiom): Fubini swap for the Schwinger integrand.
    ...
    **Proof:**
    Both sides equal ∫∫∫ F over (Ioi 0) × SpaceTime STDimension × SpaceTime STDimension by Fubini-Tonelli.
...
theorem schwinger_fubini_core (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
  ...
  let F : SpaceTime STDimension → SpaceTime STDimension → ℝ → ℂ := fun x y s =>
    (starRingEnd ℂ (f x)) * f y *
      (Real.exp (-s * m^2) : ℂ) * heatKernelPositionSpace s ‖timeReflection x - y‖

  -- Define product functions for LHS and RHS orderings
  let fL : SpaceTime STDimension × (SpaceTime STDimension × ℝ) → ℂ := fun p => F p.1 p.2.1 p.2.2
  let fR : ℝ × (SpaceTime STDimension × SpaceTime STDimension) → ℂ := fun q => F q.2.1 q.2.2 q.1
  ...
  have h_int_xys : Integrable fL
      ((volume : Measure (SpaceTime STDimension)).prod (volume.prod (volume.restrict (Set.Ioi 0)))) := by
  ...
  have hLHS : ∫ x, ∫ y, ∫ s in Set.Ioi 0, F x y s ∂volume ∂volume ∂volume =
      ∫ p, fL p ∂((volume : Measure (SpaceTime STDimension)).prod (volume.prod (volume.restrict (Set.Ioi 0)))) := by
    -- Convert inner ∫y ∫s → ∫(y,s) using Fubini
    have inner_fubini : ∀ᵐ x ∂(volume : Measure (SpaceTime STDimension)),
  ...
  calc ∫ x, ∫ y, ∫ s in Set.Ioi 0, F x y s ∂volume ∂volume ∂volume
      = ∫ p, fL p ∂((volume : Measure (SpaceTime STDimension)).prod (volume.prod (volume.restrict (Set.Ioi 0)))) := hLHS
    _ = ∫ p, fR (schwinger_tripleReorder p)
          ∂((volume : Measure (SpaceTime STDimension)).prod (volume.prod (volume.restrict (Set.Ioi 0)))) := rfl
...
/-- **THEOREM** (formerly axiom): The triple integral can be computed in either order.
    ...
    **Proof sketch:**
    ...
    The key steps:
    1. Rewrite both sides as integrals over ℝ × SpaceTime STDimension × SpaceTime STDimension
    2. Apply Fubini to swap the order of integration
    3. Use the integrability hypothesis to justify the swap -/
theorem schwinger_fubini_swap (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
    ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
      (starRingEnd ℂ (f x)) * f y *
        (∫ s in Set.Ioi 0, (Real.exp (-s * m^2) : ℂ) *
  ...
  have h_pull_in : ∀ x y : SpaceTime STDimension,
      (starRingEnd ℂ (f x)) * f y *
        (∫ s in Set.Ioi 0, (Real.exp (-s * m^2) : ℂ) *
          heatKernelPositionSpace s ‖timeReflection x - y‖) =
      ∫ s in Set.Ioi 0, (starRingEnd ℂ (f x)) * f y *
  ...
  -- both equal the triple integral ∫∫∫ F over (Ioi 0) × SpaceTime STDimension × SpaceTime STDimension.
  --
  -- The formal proof requires showing:
  ...
  -- Define the integrand function
  set F : ℝ → SpaceTime STDimension → SpaceTime STDimension → ℂ :=
...
lemma freeCovariance_eq_schwingerRep (m : ℝ) (hm : 0 < m) (x y : SpaceTime STDimension)
...
-/
theorem bilinear_schwinger_eq_heatKernel (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension) :
    ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
  ...
  -- The set {(x,y) : Θx = y} has measure zero in SpaceTime STDimension × SpaceTime STDimension
  ...
  have h_ae : ∀ᵐ x ∂(volume : Measure (SpaceTime STDimension)), ∀ᵐ y ∂volume,
      (starRingEnd ℂ (f x)) * (freeCovariance m (timeReflection x) y : ℂ) * f y =
      (starRingEnd ℂ (f x)) * f y *
        (∫ s in Set.Ioi 0, (Real.exp (-s * m^2) : ℂ) *
          heatKernelPositionSpace s ‖timeReflection x - y‖) := by
    ...
    have h_singleton : (volume : Measure (SpaceTime STDimension)) {timeReflection x} = 0 :=
      MeasureTheory.NoAtoms.measure_singleton (timeReflection x)
    -- Show: ∀ᵐ y, y ≠ Θx
    have h_compl : ∀ᵐ y ∂(volume : Measure (SpaceTime STDimension)), y ≠ timeReflection x := by
      rw [ae_iff]
  ...
  have lhs_eq : ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
        (starRingEnd ℂ (f x)) * (freeCovariance m (timeReflection x) y : ℂ) * f y =
      ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
        (starRingEnd ℂ (f x)) * f y *
          (∫ s in Set.Ioi 0, (Real.exp (-s * m^2) : ℂ) *
...
-/
theorem heatKernel_bilinear_to_mixed_rep (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension)
  ...
  -- Key proven lemmas:
  have h_hk_eq : ∀ s : ℝ, 0 < s → ∀ z : SpaceTime STDimension,
      (heatKernelPositionSpace s ‖z‖ : ℂ) =
      (1 / (2 * π) ^ STDimension : ℝ) *
      ∫ k : SpaceTime STDimension, Complex.exp (-Complex.I * ⟪k, z⟫_ℝ) * Complex.exp (-(s : ℂ) * ‖k‖^2) :=
    fun s hs z => heatKernel_eq_gaussianFT s hs z
  ...
  Step 1: Substitute `heatKernel_eq_gaussianFT` for H(s, |z|)
    LHS becomes: ∫_s e^{-sm²} ∫_x ∫_y f̄(x) f(y) · (1/(2π)^2) · ∫_k e^{-ik·z} e^{-s|k|²}
  ...
  have h_inner : ∀ k z : SpaceTime STDimension,
  ...
  have h_exp_factor : ∀ k z : SpaceTime STDimension,
  ...
  have h_norm : ∀ k : SpaceTime STDimension, ‖k‖^2 = (k 0)^2 + ‖spatialPart k‖^2 :=
  ...
  **Stage 1:** Substitute h_hk_eq to get:
    LHS = ∫_s e^{-sm²} ∫_x ∫_y f̄(x)f(y) · (1/(2π)^2) · ∫_k e^{-ik·z} e^{-s|k|²}
    where z = Θx - y
  ...
  **Stage 7:** Verify normalization:
  ...
  -- For s > 0: H(s, |z|) = (1/(2π)^2) ∫_k e^{-ik·z} e^{-s|k|²}
  ...
  have h_k_decomp : ∀ (s : ℂ) (k z : SpaceTime STDimension),
  ...
  have h_time_component : ∀ (x y : SpaceTime STDimension),
  ...
  -- Combined with normalization: (1/(2π)^2) × π = 1/(2(2π)¹)
...
/-- **THEOREM**: The Bessel bilinear form equals the mixed representation form.
    ...
    **Proof outline** (directly at bilinear level, using axioms):
    ...
    3. **Decompose k = (k₀, k_sp)**: The 2D k-integral becomes product of 1D and 1D integrals
...
lemma mixed_rep_to_k0_inside_integrand (k_spatial : SpatialCoords STDimension) (m : ℝ) [Fact (0 < m)]
    (t : ℝ) :
    let ω := Real.sqrt (‖k_spatial‖^2 + m^2)
...
theorem bilinear_to_k0_inside (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ STDimension)
    (hf_supp : ∀ x, x 0 ≤ 0 → f x = 0) :
  ∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension,
    (starRingEnd ℂ (f x)) *
    (freeCovariance m (timeReflection x) y : ℂ) *
