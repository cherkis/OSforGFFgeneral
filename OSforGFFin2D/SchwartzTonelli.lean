...
lemma spacetimeDecomp_symm_norm_ge (t : ℝ) (v : SpatialCoords STDimension) :
...
lemma schwartz_slice_integrable (f : SchwartzMap (SpaceTime STDimension) ℂ) (t : ℝ) :
    Integrable (fun v : SpatialCoords STDimension => ‖f (spacetimeDecomp.symm (t, v))‖) volume := by
  have hST_dim : Module.finrank ℝ (SpaceTime STDimension) < 5 := by
  ...
  have h_dom_integrable : Integrable (fun v : SpatialCoords STDimension => C / (1 + ‖v‖)^5) volume := by
    have h_dim : (Module.finrank ℝ (SpatialCoords STDimension) : ℝ) < 5 := by
      simp only [SpatialCoords, finrank_euclideanSpace, Fintype.card_fin]; norm_num
    have h_int := integrable_one_add_norm (E := SpatialCoords STDimension) (μ := volume) (r := 5) h_dim
    have h_eq : ∀ v : SpatialCoords STDimension, C / (1 + ‖v‖) ^ 5 = C * (1 + ‖v‖) ^ (-(5 : ℝ)) := by
      intro v
  ...
  have h_bound : ∀ v : SpatialCoords STDimension, ‖f (spacetimeDecomp.symm (t, v))‖ ≤ C / (1 + ‖v‖)^5 := by
    intro v
  ...
  · have h1 : Measurable (fun v : SpatialCoords STDimension => ((t, v) : ℝ × SpatialCoords STDimension)) :=
    ...
    have h4 : Continuous (fun x : SpaceTime STDimension => ‖f x‖) := h3.norm
...
lemma schwartz_integrable_on_prod' (f : SchwartzMap (SpaceTime STDimension) ℂ) :
    Integrable (fun p : ℝ × SpatialCoords STDimension => ‖f (spacetimeDecomp.symm p)‖) := by
  have h_mp : MeasurePreserving spacetimeDecomp (volume : Measure (SpaceTime STDimension)) volume :=
    spacetimeDecomp_measurePreserving
  have h_int : Integrable (fun x : SpaceTime STDimension => ‖f x‖) := f.integrable.norm
...
theorem schwartz_tonelli_spacetime
    (f g : SchwartzMap (SpaceTime STDimension) ℂ)
    (K : ℝ → ℝ → ℝ)
    (hK_nn : ∀ t₁ t₂, 0 ≤ K t₁ t₂)
    (hK_meas : Measurable (Function.uncurry K))
    (hK_bdd : ∃ C : ℝ, ∀ t₁ t₂, K t₁ t₂ ≤ C) :
    let G_f := fun t => ∫ (v : SpatialCoords STDimension), ‖f (spacetimeDecomp.symm (t, v))‖
    let G_g := fun t => ∫ (v : SpatialCoords STDimension), ‖g (spacetimeDecomp.symm (t, v))‖
    (∫ x : SpaceTime STDimension, ∫ y : SpaceTime STDimension, ‖f x‖ * ‖g y‖ * K (x 0) (y 0)) =
    (∫ t₁ : ℝ, ∫ t₂ : ℝ, K t₁ t₂ * G_f t₁ * G_g t₂) := by
  intro G_f G_g

  -- Step 1: Rewrite x 0 as (spacetimeDecomp x).1
  have h_time : ∀ x : SpaceTime STDimension, x 0 = (spacetimeDecomp x).1 := by
  ...
  have h_comp_symm : ∀ G : SpaceTime STDimension → ℝ,
      ∫ x : SpaceTime STDimension, G x = ∫ p : ℝ × SpatialCoords STDimension, G (spacetimeDecomp.symm p) := by
    intro G
    have h_comp : ∀ F : ℝ × SpatialCoords STDimension → ℝ,
        ∫ x : SpaceTime STDimension, F (spacetimeDecomp x) = ∫ p : ℝ × SpatialCoords STDimension, F p :=
  ...
  -- Integrability facts
  have hf_int : Integrable (fun p : ℝ × SpatialCoords STDimension => ‖f (spacetimeDecomp.symm p)‖) :=
    schwartz_integrable_on_prod' f
  have hg_int : Integrable (fun p : ℝ × SpatialCoords STDimension => ‖g (spacetimeDecomp.symm p)‖) :=
    schwartz_integrable_on_prod' g

  -- Step 4: Split each pair (t, v) and apply Fubini
  have h_split : ∀ F : ℝ × SpatialCoords STDimension → ℝ, Integrable F →
    ∫ p : ℝ × SpatialCoords STDimension, F p = ∫ t : ℝ, ∫ v : SpatialCoords STDimension, F (t, v) := by
    intro F hF
    have h_eq : (volume : Measure (ℝ × SpatialCoords STDimension)) =
        (volume : Measure ℝ).prod (volume : Measure (SpatialCoords STDimension)) := rfl
    rw [h_eq, MeasureTheory.integral_prod _ hF]

  -- The outer integrand as a function of p₁
  have h_outer_int : Integrable (fun p₁ : ℝ × SpatialCoords STDimension =>
      ∫ p₂ : ℝ × SpatialCoords STDimension,
        ‖f (spacetimeDecomp.symm p₁)‖ * ‖g (spacetimeDecomp.symm p₂)‖ * K p₁.1 p₂.1) := by
    obtain ⟨C, hC⟩ := hK_bdd
    have hC_pos : 0 ≤ C := le_trans (hK_nn 0 0) (hC 0 0)
    have h_prod_int : Integrable (fun p : (ℝ × SpatialCoords STDimension) × (ℝ × SpatialCoords STDimension) =>
        ‖f (spacetimeDecomp.symm p.1)‖ * ‖g (spacetimeDecomp.symm p.2)‖)
        ((volume : Measure (ℝ × SpatialCoords STDimension)).prod volume) :=
      hf_int.mul_prod hg_int
    have h_bound_int : Integrable (fun p : (ℝ × SpatialCoords STDimension) × (ℝ × SpatialCoords STDimension) =>
        C * (‖f (spacetimeDecomp.symm p.1)‖ * ‖g (spacetimeDecomp.symm p.2)‖))
        ((volume : Measure (ℝ × SpatialCoords STDimension)).prod volume) :=
      h_prod_int.const_mul C
    have h_meas : AEStronglyMeasurable (fun p : (ℝ × SpatialCoords STDimension) × (ℝ × SpatialCoords STDimension) =>
        ‖f (spacetimeDecomp.symm p.1)‖ * ‖g (spacetimeDecomp.symm p.2)‖ * K p.1.1 p.2.1)
        ((volume : Measure (ℝ × SpatialCoords STDimension)).prod volume) := by
      have h1 := h_prod_int.aestronglyMeasurable
      have h2 : AEStronglyMeasurable (fun p : (ℝ × SpatialCoords STDimension) × (ℝ × SpatialCoords STDimension) => K p.1.1 p.2.1)
          ((volume : Measure (ℝ × SpatialCoords STDimension)).prod volume) := by
        have hK_comp : Measurable (fun p : (ℝ × SpatialCoords STDimension) × (ℝ × SpatialCoords STDimension) => (p.1.1, p.2.1)) :=
      ...
      exact h1.mul h2
    have h_int_product : Integrable (fun p : (ℝ × SpatialCoords STDimension) × (ℝ × SpatialCoords STDimension) =>
        ‖f (spacetimeDecomp.symm p.1)‖ * ‖g (spacetimeDecomp.symm p.2)‖ * K p.1.1 p.2.1)
        ((volume : Measure (ℝ × SpatialCoords STDimension)).prod volume) := by
  ...
  have h_inner_int : ∀ t₁ v₁, Integrable (fun p₂ : ℝ × SpatialCoords STDimension =>
      ‖f (spacetimeDecomp.symm (t₁, v₁))‖ * ‖g (spacetimeDecomp.symm p₂)‖ * K t₁ p₂.1) := by
    intro t₁ v₁
    have hg_K_int : Integrable (fun p₂ : ℝ × SpatialCoords STDimension =>
      ...
      have h_meas : AEStronglyMeasurable (fun p₂ : ℝ × SpatialCoords STDimension =>
        ...
        have h2 : AEStronglyMeasurable (fun p₂ : ℝ × SpatialCoords STDimension => K t₁ p₂.1) volume := by
          have h : Measurable (fun p₂ : ℝ × SpatialCoords STDimension => (t₁, p₂.1)) :=
  ...
  have h_swap : ∀ t₁ : ℝ,
    ∫ v₁ : SpatialCoords STDimension, ∫ t₂ : ℝ, ∫ v₂ : SpatialCoords STDimension,
      ‖f (spacetimeDecomp.symm (t₁, v₁))‖ * ‖g (spacetimeDecomp.symm (t₂, v₂))‖ * K t₁ t₂ =
    ∫ t₂ : ℝ, ∫ v₁ : SpatialCoords STDimension, ∫ v₂ : SpatialCoords STDimension,
    ...
    have hGg_int : Integrable G_g := by
      have h_prod : Integrable (fun p : ℝ × SpatialCoords STDimension => ‖g (spacetimeDecomp.symm p)‖) :=
        schwartz_integrable_on_prod' g
      have h_vol_eq : (volume : Measure (ℝ × SpatialCoords STDimension)) =
          (volume : Measure ℝ).prod (volume : Measure (SpatialCoords STDimension)) := rfl
  ...
  let a := fun v₁ : SpatialCoords STDimension => ‖f (spacetimeDecomp.symm (t₁, v₁))‖
  let b := fun v₂ : SpatialCoords STDimension => ‖g (spacetimeDecomp.symm (t₂, v₂))‖
