...
lemma freeCovarianceFormR_bilinear_expand (f g : TestFunction STDimension) (t s : ℝ) :
...
lemma gff_cf_two_testfunctions (f g : TestFunction STDimension) (t s : ℝ) :
...
lemma gff_two_param_analytic (f g : TestFunction STDimension) :
...
lemma gff_slice_analytic_z0 (f g : TestFunction STDimension) (t : ℂ) :
...
lemma gff_slice_analytic_z1 (f g : TestFunction STDimension) (z₀ : ℂ) :
    AnalyticOnNhd ℂ (fun z₁ : ℂ =>
...
lemma gaussian_rhs_slice_analytic_z0 (f g : TestFunction STDimension) (t : ℂ) :
    AnalyticOnNhd ℂ (fun z₀ : ℂ =>
...
lemma gaussian_rhs_slice_analytic_z1 (f g : TestFunction STDimension) (z₀ : ℂ) :
    AnalyticOnNhd ℂ (fun z₁ : ℂ =>
      Complex.exp (-(1/2 : ℂ) * (z₀^2 * freeCovarianceFormR m f f +
...
lemma gff_cf_agrees_on_reals_OS0 (f g : TestFunction STDimension) (t s : ℝ) :
...
theorem schwinger_eq_covariance_real (f g : TestFunction STDimension) :
    ∫ ω, (ω f) * (ω g) ∂(gaussianFreeField_free m).toMeasure =
      freeCovarianceFormR m f g := by
  -- Use polarization identity: XY = ¼((X+Y)² - (X-Y)²)
  have h_polar : ∀ ω : FieldConfiguration STDimension,
...
lemma schwinger_eq_covarianceℂ_on_reals (f g : TestFunction STDimension) :
...
theorem gff_two_point_equals_covarianceℂ_free (m : ℝ) [Fact (0 < m)] (f g : TestFunctionℂ STDimension) :
