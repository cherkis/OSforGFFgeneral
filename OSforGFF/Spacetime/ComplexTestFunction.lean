...
lemma ω_re_decompose_linear
  (ω : FieldConfiguration4) (f g : TestFunctionℂ4) (t s : ℂ) :
  ...
  have := congrArg (fun (φ : TestFunction4) => ω φ) h_sum_re_eq
...
lemma ω_im_decompose_linear
  ...
  -- Apply ω (a real-linear functional) to both sides
  have := congrArg (fun (φ : TestFunction4) => ω φ) h_sum_im_eq
  -- Simplify using linearity of ω over ℝ
...
lemma iteratedFDeriv_ofReal_norm_eq (f : TestFunction4) (n : ℕ) (x : SpaceTime4) :
...
def toComplex (f : TestFunction4) : TestFunctionℂ4 :=
...
@[simp] lemma toComplex_apply (f : TestFunction4) (x : SpaceTime4) :
  toComplex f x = (f x : ℂ) := by
  -- Follows from definition of toComplex
  rfl

@[simp] lemma complex_testfunction_decompose_toComplex_fst (f : TestFunction4) :
  (complex_testfunction_decompose (toComplex f)).1 = f := by
  ext x
  simp [complex_testfunction_decompose, toComplex_apply]

@[simp] lemma complex_testfunction_decompose_toComplex_snd (f : TestFunction4) :
  (complex_testfunction_decompose (toComplex f)).2 = 0 := by
  ext x
  simp [complex_testfunction_decompose, toComplex_apply]

@[simp] lemma toComplex_add (f g : TestFunction4) :
  toComplex (f + g) = toComplex f + toComplex g := by
  ext x
  simp [toComplex_apply]

@[simp] lemma toComplex_smul (c : ℝ) (f : TestFunction4) :
...
noncomputable def toComplexCLM : TestFunction4 →L[ℝ] TestFunctionℂ4 :=
...
lemma distributionPairingℂ_real_conj (ω : FieldConfiguration4) (f : TestFunctionℂ4) :
