...
lemma gff_exp_pairing_integrable (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4) :
...
lemma gff_exp_time_translated_memLp_two (m : ℝ) [Fact (0 < m)] (s : ℝ) (f : TestFunctionℂ4) :
    MemLp (fun ω : FieldConfiguration4 =>
        Complex.exp (distributionPairingℂ_real (timeTranslationDistribution s ω) f))
  ...
  have h_meas : AEStronglyMeasurable
      (fun ω : FieldConfiguration4 => Complex.exp (distributionPairingℂ_real ω g))
      (gaussianFreeField_free m).toMeasure := by
  ...
  have h_dom : Integrable (fun ω : FieldConfiguration4 =>
  ...
  have h_sq_norm_bound : ∀ ω : FieldConfiguration4,
...
lemma timeTranslationSchwartzℂ_conj_comm (t : ℝ) (f : TestFunctionℂ4) :
...
lemma gff_exp_product_time_shift_invariant (m : ℝ) [Fact (0 < m)] (g₁ g₂ : TestFunctionℂ4) (t : ℝ) :
    let μ := (gaussianFreeField_free m).toMeasure
  ...
  have h_add : ∀ ω (f g : TestFunctionℂ4),
  ...
  simp_rw [h_add]
  -- T_t f + T_t g = T_t(f + g)
  have h_T_add : ∀ (f g : TestFunctionℂ4),
...
lemma gff_exp_L2_norm_constant (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4) (s : ℝ) :
  ...
  have h_int_re_eq : ∀ g : TestFunctionℂ4,
...
lemma time_average_memLp_two (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4) (T : ℝ) (hT : T > 0) :
  ...
  have h_avg_meas : AEStronglyMeasurable
    ...
    have h_swap : AEStronglyMeasurable (fun (p : FieldConfiguration4 × ℝ) => A p.2 p.1)
...
lemma gff_err_sq_integrable (m : ℝ) [Fact (0 < m)] (T : ℝ) (hT : T > 0) (f : TestFunctionℂ4) :
    Integrable (fun ω =>
  ...
  have h_const_L2 : MemLp (fun _ : FieldConfiguration4 => EA) 2 μ := memLp_const EA
  ...
  have h_EA : ∫ ω' : FieldConfiguration4, Complex.exp (distributionPairingℂ_real ω' f) ∂μ = EA := by
    simp only [EA, A]
    congr 1
    ext ω'
    congr 2
...
lemma gff_product_expectation_stationarity (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4)
    (s u : ℝ) :
    let μ := (gaussianFreeField_free m).toMeasure
...
lemma gff_covariance_timeTranslation_continuous (m : ℝ) [Fact (0 < m)]
  ...
  have h_bound_int : Integrable bound' (volume.prod volume) := by
    ...
    have hg_int : Integrable (fun y => ‖g y‖) (volume : Measure SpaceTime4) :=
      (SchwartzMap.integrable (μ := volume) g).norm
    have hK_norm : Integrable (fun z => ‖(freeCovarianceKernel m z : ℂ)‖) (volume : Measure SpaceTime4) := by
      have := hK_int.norm
      simp only [Real.norm_eq_abs] at this
      convert this using 1
      ext z; simp
    have h_eq3 : (fun p : SpaceTime4 × SpaceTime4 => ‖(freeCovarianceKernel m (p.1 - p.2) : ℂ)‖ * ‖g p.2‖) =
...
lemma gff_covariance_continuous (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4) :
    let μ := (gaussianFreeField_free m).toMeasure
    let A := fun t ω => Complex.exp (distributionPairingℂ_real (timeTranslationDistribution t ω) f)
...
lemma L2_time_average_variance_bound (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4) (T : ℝ) (hT : T > 0) :
    let μ := (gaussianFreeField_free m).toMeasure
    let A := fun s ω => Complex.exp (distributionPairingℂ_real (timeTranslationDistribution s ω) f)
    let EA := ∫ ω, Complex.exp (distributionPairingℂ_real ω f) ∂μ
  ...
  have h_Fubini : Integrable (fun (x : FieldConfiguration4 × (ℝ × ℝ)) =>
...
lemma clustering_implies_covariance_decay (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4)
...
lemma gff_covariance_norm_integrableOn_slice (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4)
    (s : ℝ) (T : ℝ) :
    let μ := (gaussianFreeField_free m).toMeasure
    let A := fun t ω => Complex.exp (distributionPairingℂ_real (timeTranslationDistribution t ω) f)
    let EA := ∫ ω, Complex.exp (distributionPairingℂ_real ω f) ∂μ
...
lemma variance_decay_from_clustering (m : ℝ) [Fact (0 < m)] (f : TestFunctionℂ4)
    (c : ℝ) (hc : c ≥ 0)
...
theorem OS4'_implies_OS4 (m : ℝ) [Fact (0 < m)] :
  ...
  let Err : Fin n → ℝ → FieldConfiguration4 → ℂ := fun j T ω =>
  ...
  have h_upper : ∀ T > 0, ∫ ω, ‖(1 / T) * ∫ s in Set.Icc (0 : ℝ) T, A (timeTranslationDistribution s ω)
    ...
    have h_err_eq : ∀ ω, (1 / T) * ∫ s in Set.Icc (0 : ℝ) T, A (timeTranslationDistribution s ω)
      ...
      have h_mean_sum : ∫ ω', ∑ j, z j * Complex.exp (distributionPairingℂ_real ω' (f j)) ∂μ =
        ...
        have h_icm : ∀ (c : ℂ) (g : FieldConfiguration4 → ℂ),
            ∫ a, c * g a ∂μ = c * ∫ a, g a ∂μ :=
